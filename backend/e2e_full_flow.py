...
def test_us04_us13_us26_agreement_upload_and_validate():
    ...
    with sync_playwright() as p:
        ...
        if "Goedgekeurd" in content:
            ...
            if "Upload" in content or "upload" in content:
                ...
                if file_input.count() > 0:
                    ...
                    assert (
                        "Geüpload" in content
                        or "succesvol" in content
                        or "Ingediend" in content
        ...
        if admin_overeenkomsten:
            ...
            content = page.locator("#content").inner_text()
            assert (
                "Overeenkomsten" in content or "Overzicht" in content
            ), "Admin agreements not found"
...
def test_us05_us08_us21_us22_logbook_flow():
    ...
    with sync_playwright() as p:
        ...
        for cell in week_cells:
            ...
            for cell in week_cells:
                ...
                if "Concept" in text:
                    ...
                    if form.count() > 0 and form.is_visible():
                        page.locator("#log-tasks").fill("E2E test taken deze week")
                        page.locator("#log-reflection").fill(
                            "E2E reflectie over de week"
                        )
                        page.locator("#log-issues").fill("E2E problemen of leerpunten")
                        page.locator('#logbook-form button[type="submit"]').click()
                        page.wait_for_timeout(1500)
        ...
        if validatie_tab:
            ...
            content = page.locator("#content").inner_text()
            assert (
                "Validatie" in content or "Logboek" in content
            ), "Mentor validation view not found"
...
def test_us15_us17_teacher_followup_and_evaluation():
    ...
    with sync_playwright() as p:
        ...
        if opvolging_tab:
            ...
            content = page.locator("#content").inner_text()
            assert (
                "Logboeken" in content or "Opvolging" in content
            ), "Teacher logbook view not found"
...
def test_us09_us18_us19_final_report_and_evaluation():
    ...
    with sync_playwright() as p:
        ...
        content = page.locator("#content").inner_text()
        assert (
            "Evaluaties" in content or "Eindoverzicht" in content
        ), "Student evaluations not found"
...
def test_us25_admin_competencies():
    ...
    with sync_playwright() as p:
        ...
        if comp_tab:
            ...
            content = page.locator("#content").inner_text()
            assert (
                "Competentie" in content or "Competenties" in content
            ), "Competency view not found"
...
def main():
    ...
    print(f"\n{'='*50}")
    print(
        f"E2E Results: {passed} passed, {failed} failed out of {len(tests)} test groups"
    )
    if failed > 0:
        sys.exit(1)
